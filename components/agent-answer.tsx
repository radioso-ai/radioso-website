...
type Props = {
  ...
  streaming?: boolean
  /**
   * `card` = standalone surface. `inline` = no chrome, used inside FAQ rows.
   * `chat` = message bubbles for the hero window, which supplies its own frame.
   */
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-3">
            ...
            {data.sources.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {data.sources.map((s) => (
                  <SourceChip key={s.n} source={s} compact />
                ))}
              </div>
            )}
            <AnsweredBy />
...
function SourceChip({ source, compact }: { source: AgentSource; compact?: boolean }) {
  return (
    <span
      className={cn(
        'inline-flex items-center gap-2 rounded-full border border-border bg-card px-2.5 py-1 text-[11px] text-muted-foreground',
